    ...
    <el-dialog v-model="centerDialogVisible" title="居民楼信息修改" width="35%" :append-to-body=true :draggable=true :modal-append-to-body=false :z-index="1000" align-center>
        <el-form :model="detailedForm">
            ...
            <el-row>
                <el-col :span="24">
                    <el-form-item label="建筑名称" :label-width="formLabelWidth">
                        <el-input v-model="detailedForm.buildName" autocomplete="off" placeholder="请输入" />
                    </el-form-item>
                </el-col>
            </el-row>
            <el-row>
...
// 引入查询账户信息接口
import {selectAllBuildDataForLMAPI} from '../../../http/api/buildAPI.js'
// 引入修改建筑信息接口
    ...
    setup () {
        ...
        const queryUserData = (formEl: FormInstance | undefined) => { //查询按钮
            ...
            formEl.validate((valid) => {
                if (valid) {
                    selectAllBuildDataForLMAPI(selectDataValue).then(res =>{ //动态查询数据并渲染到表格中
        ...
        //数据展示
        const sellectAllBuildData = selectAllBuildDataForLMAPI(selectDataValue).then(res =>{ //数据显示
        ...
        const CurrentChange = (currentPage: any) =>{ //分页改变当前页数
            ...
            selectAllBuildDataForLMAPI(selectDataValue).then(res =>{
                dataValue.allData = res.data;
            })
        }
                ...
                updateBuildInfByIdAPI(detailedForm).then(res =>{
                    ...
                    if(state == 200){
                        if(result == 1){
                            nextTick(() => { // 刷新页面（局部）
                                router.replace({
                                    path: '/redirect' + '/BuildManageMent_LM',
                                })
                            });
                            ElNotification({
                                title: '操作成功！',
