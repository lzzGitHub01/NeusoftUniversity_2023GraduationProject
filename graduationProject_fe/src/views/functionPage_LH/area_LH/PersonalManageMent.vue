...
// 引入查询人员信息接口
import {selectAllPersonnelDataAPI} from '../../../http/api/personalAPI.js'
    ...
    setup () {
        ...
        const queryPersonalData = (formEl: FormInstance | undefined) => { //查询按钮
            ...
            formEl.validate((valid) => {
                if (valid) {
                    selectAllPersonnelDataAPI(selectDataValue).then(res =>{ //动态查询数据并渲染到表格中
        ...
        //数据展示
        const sellectAllPersonnelData = selectAllPersonnelDataAPI(selectDataValue).then(res =>{ //数据显示
        ...
        const CurrentChange = (currentPage: any) =>{ //分页改变当前页数
            ...
            selectAllPersonnelDataAPI(selectDataValue).then(res =>{
                dataValue.allData = res.data;
            })
        }
