            ...
            <el-row justify="space-between">
                <el-col :span="24">
                    <el-form-item label="七日内行程" label-width="83px">
                        <el-input v-model="detailForm.sevenDayTrip" maxlength="100" placeholder="请选择" show-word-limit style="width:686px" type="textarea" disabled/>
                    </el-form-item>
                </el-col>
            </el-row>
        </el-form>
...
// 引入查询人员信息接口
import {selectAllPersonnelDataForLNAPI} from '../../../http/api/personalAPI.js'
    ...
    setup () {
        ...
        const queryPersonalData = (formEl: FormInstance | undefined) => { //查询按钮
            ...
            formEl.validate((valid) => {
                if (valid) {
                    selectAllPersonnelDataForLNAPI(selectDataValue).then(res =>{ //动态查询数据并渲染到表格中
        ...
        //数据展示
        const sellectAllPersonnelData = selectAllPersonnelDataForLNAPI(selectDataValue).then(res =>{ //数据显示
        ...
        const CurrentChange = (currentPage: any) =>{ //分页改变当前页数
            ...
            selectAllPersonnelDataForLNAPI(selectDataValue).then(res =>{
                dataValue.allData = res.data;
            })
        }
