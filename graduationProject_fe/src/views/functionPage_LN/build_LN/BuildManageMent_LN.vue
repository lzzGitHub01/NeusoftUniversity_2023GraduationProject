<template>
            ...
            <el-table-column fixed="right" prop="" label="操作" width="242">
                <template #default="scope">
                    ...
                    <el-popconfirm width="200" auto-insert-space confirm-button-text="是" cancel-button-text="否" title="是否删除当前数据" icon-color="#626AEF" @confirm="confirmEvent(scope.row,scope.$index)" @cancel="cancelEvent">
                        <template #reference>
                            <el-button type="danger" size="small" disabled><el-icon><Delete /></el-icon>删除</el-button>
                        </template>
                    </el-popconfirm>
                </template>
            </el-table-column>
    ...
    <!-- 修改按键的Dialog页面 -->
    <el-dialog v-model="centerDialogVisible" title="居民楼信息修改" width="35%" :append-to-body=true :draggable=true :modal-append-to-body=false align-center>
        ...
        <template #footer>
            <span class="dialog-footer">
                <el-button type="primary" @click="updateBuildInf()">保存</el-button>
    ...
    <!-- 新增按键的Dialog页面 -->
    <el-dialog v-model="addDialogVisible" title="新增居民楼" width="35%" :append-to-body=true :draggable=true :modal-append-to-body=false align-center>
        ...
        <template #footer>
            <span class="dialog-footer">
                <el-button type="primary" @click="addNewBuild()">保存</el-button>
...
// 引入查询账户信息接口
import {selectAllBuildDataForLNAPI} from '../../../http/api/buildAPI.js'
// 引入修改建筑信息接口
    ...
    setup () {
        ...
        const queryUserData = (formEl: FormInstance | undefined) => { //查询按钮
            ...
            formEl.validate((valid) => {
                if (valid) {
                    selectAllBuildDataForLNAPI(selectDataValue).then(res =>{ //动态查询数据并渲染到表格中
        ...
        //数据展示
        const sellectAllBuildData = selectAllBuildDataForLNAPI(selectDataValue).then(res =>{ //数据显示
        ...
        const CurrentChange = (currentPage: any) =>{ //分页改变当前页数
            ...
            selectAllBuildDataForLNAPI(selectDataValue).then(res =>{
                dataValue.allData = res.data;
            })
        }
        ...
        const updateBuildInf = () =>{// 修改页面的保存按键
            updateBuildInfByIdAPI(detailedForm).then(res =>{
                ...
                if(state == 200){
                    if(result == 1){
                        nextTick(() => { // 刷新页面（局部）
                            router.replace({
                                path: '/redirect' + '/BuildManageMent_LN',
                            })
                        });
                        ElNotification({
                            title: '操作成功！',
